...
class ProtoInjector(nn.Module):
    """
    Single-Head Cross Attention
    """
    def __init__(self, channels, proto_dim=512):
        super().__init__()
        self.channels = channels

        if proto_dim != channels:
            self.proto_proj = nn.Linear(proto_dim, channels)
        else:
    ...
    def forward(self, x, prototypes):
        ...
        if prototypes.dim() == 3:
            all_protos = prototypes.flatten(0, 1)
        else:
            all_protos = prototypes

        k_feat = self.proto_proj(all_protos)
        v_feat = k_feat 

        # [B, C, H, W] -> [B, HW, C]
        q_feat = x.flatten(2).transpose(1, 2)

        attn_score = torch.matmul(q_feat, k_feat.t()) * self.scale
        attn_weight = F.softmax(attn_score, dim=-1)
        out = torch.matmul(attn_weight, v_feat)
...
def _xyxyn_to_xyxy_abs(boxes, W, H):
    """
...
class VisualPrototypeBank(nn.Module):
    def __init__(self, dim=128, momentum=0.9, k=3, warmup_steps=0):
        super().__init__()
        self.dim = dim
        ...
        self.num_viewpoints = 7 
        self.viewpoint_start_idx = 1 
        
        self.register_buffer("prototypes", torch.zeros(self.num_viewpoints, k, dim))
        self.register_buffer("filled_count", torch.zeros(self.num_viewpoints, dtype=torch.long))
        self.register_buffer("initialized", torch.zeros(self.num_viewpoints, dtype=torch.bool))
        self.register_buffer("global_step", torch.zeros(1, dtype=torch.long))

    def update_with_masks(
        self,
        features,
        batch_masks,
        batch_idx,
        ...
        if bbox_format == "xywhn":
            boxes_abs = _xywhn_to_xyxy_abs(batch_bboxes.float(), W, H)
        elif bbox_format == "xyxyn":
            boxes_abs = _xyxyn_to_xyxy_abs(batch_bboxes.float(), W, H)
        else:
            raise ValueError(f"Unsupported bbox_format: {bbox_format}")

        
        # find all view sub-images in the mosaic augmented image
        view_records = {b: [] for b in range(B)}

        N = len(batch_cls)
        for i in range(N):
            b_id = int(batch_idx[i].item())
            c_id = int(batch_cls[i].item())

            if 1 <= c_id <= 7:
                view_records[b_id].append({
                    "view_id": c_id,
                    "bbox": boxes_abs[i]
                })

        for b_id in range(B):
            feat_map = features[b_id]      # [C,H,W]
            inst_mask_map = masks_ds[b_id] # [H,W], instance-id mask

            if len(view_records[b_id]) == 0:
                continue

            for rec in view_records[b_id]:
                view_id = rec["view_id"]
                box = rec["bbox"]

                x1, y1, x2, y2 = _clip_xyxy(box, W, H)

                if x2 <= x1 or y2 <= y1:
                    continue

                sub_mask = inst_mask_map[y1:y2, x1:x2]   # [h,w]
                if sub_mask.numel() == 0:
                    continue

                bank_idx = view_id - self.viewpoint_start_idx

                # merged_local_mask = (sub_mask > 0)
                merged_local_mask = sub_mask 
                if merged_local_mask.sum() < min_pixels:
                    continue

                full_mask = torch.zeros((H, W), dtype=torch.bool, device=feat_map.device)
                full_mask[y1:y2, x1:x2] = merged_local_mask.to(feat_map.device)
                obs = self._extract_observation_from_mask(
                    feat_map,
                    full_mask,
                ...
                self._update_slot(bank_idx, obs)


    def _extract_observation_from_mask(
        self,
        feat_map,
        full_mask,
        min_pixels=1,
        ...
        valid_pixels = full_mask.bool()
        if valid_pixels.sum() < min_pixels:
            return None

        # mean pooling
        pooled_feat = feat_map[:, valid_pixels]   # [C, N]
        if pooled_feat.shape[1] == 0:
            return None
        observation = pooled_feat.mean(dim=1)
        observation = F.normalize(observation, dim=0)
        return observation


    def _update_slot(self, idx, observation):
        current_count = 0
        for i in self.prototypes[idx]:
            if i.sum()>1e-6:
                current_count += 1
            self.filled_count[idx] = current_count
            if self.filled_count[idx] >= self.k:
                self.initialized[idx] = True
        if not self.initialized[idx]:
            ptr = self.filled_count[idx].item()
            self.prototypes[idx, ptr] = observation
            self.filled_count[idx] += 1
            if self.filled_count[idx] >= self.k:
                self.initialized[idx] = True
                print(f"[Bank] View {idx+1} Initialized!")
        else:
            current_protos = self.prototypes[idx]
            sims = torch.matmul(current_protos, observation)
            winner = torch.argmax(sims).item()
            old = self.prototypes[idx, winner]
            new = old * self.momentum + observation * (1 - self.momentum)
            self.prototypes[idx, winner] = F.normalize(new, dim=0)

    def get_prototypes(self):
        return self.prototypes.view(-1, self.dim)
...
class StripBlock_SiLU(nn.Module):
    def __init__(self, dim, k1, k2):
        super().__init__()
        self.conv0 = nn.Conv2d(dim, dim, 5, padding=2, groups=dim)
        self.conv_spatial1 = nn.Conv2d(dim,dim,kernel_size=(k1, k2), stride=1, padding=(k1//2, k2//2), groups=dim)     
